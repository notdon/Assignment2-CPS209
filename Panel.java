...
public class Panel extends JPanel {
	 ...
	 */
	public void setUpVehicle(MouseEvent event) { // This just Adds Vehicle Class , Can be Car or the Truct
            
		if(vehicleObjects.size() == 0) { // when first pressed adds the trucl
			vehicleObjects.addLast(new Truck(event.getX(), event.getY()));
		} else if(vehicleObjects.size() < 6){ //when 2nd to 6th click , cares are added 
	...
	public void addFirst() {
		...
		Vehicle v2;
		if(vehicleObjects.size() >= 6){ //if all of the cars have been added 
			for(int i=0; i<vehicleObjects.size(); i++){  
				if(vehicleObjects.get(i).equals(selectedVehicle)&&!vehicleObjects.get(i).hasP()){ //If only 1 car is present 
					if(!vehicleObjects.get(0).hasN()){ //if the have no next car
						...
						vehicleObjects.get(i).setP(vehicleObjects.get(0));
                                                ...
                                                for (Vehicle v : vehicleObjects) {  //all this does is , It moves 80 away from the vehicles, As long as the vehicle has next it aparts 80 from previous
					if (v.hasN()) { // if vehicle has next
                                             for (StackBoxes s : StackBoxs) {  //check if each car has a box with it
                    ...
                    else{ // if the car has next car with it
						...
						Vehicle v = vehicleObjects.get(i);//updates the Next and previous nodes of each car
						while(v.hasN()){
							v = v.getN();
						}
						v.setN(v2);//updates the Next and previous nodes of each car
						v2.setP(v);//updates the Next and previous nodes of each car
                                                
                                                
                                                
                                                
                                                for (Vehicle vp : vehicleObjects) {  //all this does is , It moves 80 away from the vehicles, As long as the vehicle has next it aparts 80 from previous
					if (vp.hasN()) {
                                             for (StackBoxes s : StackBoxs) {
	 ...
	 */
	public void addLast() {  //adds the car to the last of truck
		if(vehicleObjects.size()>=6){ // if all vehicles are added
			for(int i=0; i<vehicleObjects.size(); i++){  
				if(vehicleObjects.get(i).equals(selectedVehicle)&&!vehicleObjects.get(i).hasP()){ //checks which car is selected and whether it has next or not 
					Vehicle v = vehicleObjects.get(0); // truck is selected 
					while(v.hasN()){ //traverses till there is not next
						v = v.getN();
					}
	...
	public class Handlerclass implements MouseListener, MouseMotionListener {
		...
		@Override
		public void mouseDragged(MouseEvent e) { // if mouse dragged this is executed 
                    moves=false;
                    
			if (selectedVehicle != null && !selectedVehicle.isTruck()) { //if atleast a car is selected and its not a car
                            //while(!selectedVehicle.hasN())
                            for (StackBoxes s : StackBoxs) { // checks all the stackboxes
				...
				//all this does is , It moves 80 away from the vehicles, As long as the vehicle has next it aparts 80 from previous
					for (Vehicle v : vehicleObjects) {
						// if v has a next car
						if (v.hasN()) {
                                            for (StackBoxes s : StackBoxs) { // check all the boxes on all the cars 
                                               if(v.getN().getRect().intersects(s.getRect()))
                                               {
						...
						v.getN().moveTo(v.getX()+80, v.getY()); //updats the vehicles 
					}
				}
			}
		...
		public void mousePressed(MouseEvent e) {
			...
			if (setUpVehicleDone()){ //Checks if vehicles are more than or less than 6
				for (Vehicle v : vehicleObjects) { 
					if (v.hasP() == false && v.getRect().contains(e.getPoint())) { //hasP means has previous
		...
		public void mouseReleased(MouseEvent e) {
			...
			if (selectedVehicle != null) {
				...
				for(Vehicle vehicle : vehicleObjects) {  // Check all the cars 
					if(vehicle.equals(selectedVehicle) == false) {
                                            
                                          
						if(vehicle.getRect().intersects(selectedVehicle.getRect())) { //Checks the proximity of the cars
                                                  
							if(vehicle.getN() == null)  { // if vehicles has next 
                                ...
                                for (Vehicle v : vehicleObjects) {  //all this does is , It moves 80 away from the vehicles, As long as the vehicle has next it aparts 80 from previous
					if (v.hasN()) { // if car has next 
                                                                  for (StackBoxes s : StackBoxs) { // if it has the stack
                                                      if(v.getN().getRect().intersects(s.getRect()))
                                                      {  
                                                           s.x=v.getX()+85;
	...
	public void paintComponent(Graphics g) {
		...
		for (Vehicle v : vehicleObjects) { //checks all the cars 
			if(v.hasP() == false) {
		...
		if (vehicleObjects.size() >= 6) {
			for (Vehicle v : vehicleObjects) {
				if (v.equals(selectedVehicle) && selectedVehicle.hasN() && selectedVehicle.getP() !=  vehicleObjects.get(0)) {//if the cars has next , and is selected , then update all the cars with red color 
					g2.setColor(Color.red);
					v.draw(g2);
				} 
			}
                 if(StackBoxs.size()!=0)
                 {
                     ...
                     blackbox.draw(g2); // if car has more than 1 stack , i.e 5 stack, then draw them
                     for (StackBoxes v : StackBoxs) {
